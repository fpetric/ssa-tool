...
* Introduction
:PROPERTIES:
:ID:       4153C750-A650-4838-BF66-4D541BF2F539
:END:
...
**** A Silly Counter
:PROPERTIES:
:noweb-ref: silly counter
:END:
#+BEGIN_SRC python
  basic_node_counter = None
  def reset_basic_node_counter():
      global basic_node_counter
      basic_node_counter = 0
  reset_basic_node_counter()
  def BasicNode:
      global basic_node_counter
      basic_node_counter += 1
      return basic_node_counter
#+END_SRC
...
*** Trees
:PROPERTIES:
:noweb-ref: random trees
:END:
**** Deep
#+BEGIN_SRC python
  def deep_tree(degree, base_class=BasicNode, **properties):
      G = random_graph(degree=degree,
                       edge_probability=0,
                       base_class=base_class,
                       ,**properties)

      while not networkx.is_connected(G):
          to_connect = random.sample(networkx.connected_components(G), 2)
...
**** TODO Broad
#+BEGIN_SRC python
  def broad_tree(degree, base_class=BasicNode, breadth=.5, **properties):
      G = random_graph(degree=degree,
                       edge_probability=0,
                       base_class=base_class,
                       ,**properties)
...
*** Sparse Graphs
:PROPERTIES:
:noweb-ref: sparse graphs
:END:

#+BEGIN_SRC python
  def sparse_graph(degree, base_class=BasicNode, extra_paths=None, **properties):
      G = deep_tree(degree=degree,
                    base_class=base_class,
                    ,**properties)
...
:PROPERTIES:
:noweb-ref: animating
:ID:       B701F108-E3E8-4C77-9C34-8EA00D5DA30A
:END:
