...
#+BEGIN_SRC sh
  ...
  python3 ssa.py test.ssax new predicate can-mark < unmarked-and-neighbors-unmarked.py
  python3 ssa.py test.ssax new move mark < mark.py

  python3 ssa.py test.ssax new predicate must-unmark < marked-and-neighbor-marked.py
  python3 ssa.py test.ssax new move unmark < unmark.py

  # create a new algorithm
  python3 ssa.py test.ssax new algorithm 'Independent Set'

  # define rules for that algorithm
  python3 ssa.py test.ssax add-rule-to 'Independent Set' can-mark mark
  python3 ssa.py test.ssax add-rule-to 'Independent Set' must-unmark unmark
