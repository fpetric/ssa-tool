...
#+BEGIN_SRC python :noweb-ref "test"
  ...
  def new_graph():
      global n
      n += 1
      grapher.set_graph(nx.complete_graph(n))
      grapher.paint()
...
#+BEGIN_SRC python
  def _queue_next_graph(self):
      ...
      # check if new graph is delta
      if self.isdelta(change):
          <<modify graph>>
      else:
          self.next_graph = change
#+END_SRC
...
#+BEGIN_SRC python :noweb-ref "modify graph"
  ...
  for key, value in data.keys(), data.values():
      self.next_graph[node][key] = value
#+END_SRC
